...
Equivalent to [group](#group) but returning a unique value instead of an array. Throws if the value is not unique.

On the data array defined above:
```js
...
`d3.index(data, d => d.name)` throws a "non-unique key" Error, because there are two objects with the same name.
